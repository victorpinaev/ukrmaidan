...
$(".post-rating").rating({
    ...
    // 'stars': '10',
    // 'step': '0.5'
    min: 0, max: 10, step: 0.5, size: "sm", stars: "10"
});
...
$(document).ready(function () {
    ...
    $('input:radio[name=options]').on('change', function () {
        $('#cat_hidden_field').val($(this).val());
        $('#post_search_form').submit();
        //window.location.href = '/?cat=' + $(this).val();
    });

    $('.post-rating').on('rating.change', function(event, value, caption) {

        $(this).rating('refresh', {disabled: true, showClear: false, showCaption: true});
        $.ajax({
            url: url,//'index.php?r=site%2Frating',
            type: 'post',
            data: {
                value: value,
